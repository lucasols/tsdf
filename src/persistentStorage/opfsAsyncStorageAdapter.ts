...
> implements AsyncStorageNamespaceHandle<TValue, TCustomMetadata> {
    ...
    try {
      ...
      for (const key of uniqueKeys) {
        ...
        resultsByKey.set(key, {
          value: payload,
          // eslint-disable-next-line @typescript-eslint/consistent-type-assertions -- Metadata custom fields are namespace-specific and retyped at the handle boundary.
          metadata: {
            ...metadata,
            ...(shouldTouch ? { lastAccessAt: now } : {}),
          } as AsyncStorageEntryMetadata<TCustomMetadata>,
        });
      }
  ...
  async listMetadata(
    ...
    try {
      ...
      return {
        // eslint-disable-next-line @typescript-eslint/consistent-type-assertions -- Metadata pages preserve namespace-specific fields but are stored internally as generic records.
        entries: nextEntries as AsyncStorageEntryMetadata<TCustomMetadata>[],
        cursor:
          cursorOrder === order
            ? nextCursor
...
export class OpfsAsyncStorageAdapter implements AsyncStorageAdapter {
  ...
  >(
    ...
    const accessBucketsDir = await this.getBucketDir(
      context.dir,
      ACCESS_BUCKETS_DIR,
    );
    const writeBucketsDir = await this.getBucketDir(
      context.dir,
      WRITE_BUCKETS_DIR,
    );
  ...
  private async performStartupCleanup(): Promise<void> {
    ...
    for (const entry of registry.entries) {
      ...
      const protectedRefs = await this.readProtectedRefs(entry.sessionKey);
      protectedRefsBySession.set(entry.sessionKey, protectedRefs);
      const metadataEntries = await this.listAllMetadata(context);
      const referencedPayloads = new Set<string>();
      const lookupDir = await context.dir.getDirectoryHandle(LOOKUP_DIR, {
        create: true,
      });
      const accessDir = await this.getBucketDir(
        context.dir,
        ACCESS_BUCKETS_DIR,
      );
      const writeDir = await this.getBucketDir(context.dir, WRITE_BUCKETS_DIR);
      const payloadsDir = await context.dir.getDirectoryHandle(PAYLOADS_DIR, {
        create: true,
      });
  ...
  private async readJsonFile<T>(
    ...
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/consistent-type-assertions -- JSON.parse returns unknown and this helper centralizes the generic file decoding step.
