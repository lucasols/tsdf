...
test('list query partial-resource snapshots do not seed an untouched sibling item tab', async () => {
  ...
  expect(pick(envAFetchEntries[0], ['fields'])).toMatchInlineSnapshot(
    `fields: ['name']`,
  );
...
test('a fresh partial-resource list-query tab still performs its first fetch after a sibling tab fetched earlier', async () => {
  ...
  const envBQuery = renderHook(() => {
    const query = envB.apiStore.useListQuery(
      { tableId: 'users' },
      { returnRefetchingStatus: true, fields: ['name'] },
    );
  ...
  expect(pick(envBQuery.result.current, ['status', 'items']))
  ...
  expect(envA.timelineString).toMatchInlineSnapshot(`
    "
    time  |
    3.01s | 🔴 >list-fetch-started
    3.81s | 🔴 <list-fetch-finished (value: {"count":2})
    7.62s | <confirmed-query-snapshot-received (value: {"queryKey":"{tableId:\\"users\\"}","itemCount":2})
    "
  `);
  expect(envB.timelineString).toMatchInlineSnapshot(`
    "
    time  | users||1 |
    3.81s | ···      | ui-initialized
    6.82s | ···      | 🔴 >list-fetch-started
    7.62s | ···      | 🔴 <list-fetch-finished (value: {"count":2})
    .     | Alice    | ui-changed
    "
  `);
});
...
test('list query partial-resources field metadata updates an already-loaded sibling tab', async () => {
  ...
  expect(pick(envBNameHook.result.current, ['status', 'data']))
    .toMatchInlineSnapshot(`
      data: null
      status: 'loading'
    `);
  ...
  envA.serverTable.setItem('users||1', { id: 1, name: 'Alicia', age: 30 });
  ...
  ).toMatchInlineSnapshot(`['age', 'name']`);
  ...
  expect(envB.timelineString).toMatchInlineSnapshot(`
    "
    time  | name-hook |
    3.01s | -         | 🔴 [users||1] >fetch-started
    3.81s | -         | 🔴 [users||1] <fetch-finished (value: {"age":30})
    .     | ···       | [name-hook] ui-initialized
    4.62s | ···       | [users||1] <confirmed-item-snapshot-received (value: {"name":"Alicia"})
...
test('focused partial-resource item refetch updates the matching background hook without refetching a sibling field hook', async () => {
  ...
  expect(pick(envANameHook.result.current, ['status', 'data']))
    .toMatchInlineSnapshot(`
      data: { name: 'Alice' }
      status: 'success'
    `);
  expect(pick(envBHooks.result.current.name, ['status', 'data']))
    .toMatchInlineSnapshot(`
      ...
      status: 'success'
    `);
  expect(pick(envBHooks.result.current.age, ['status', 'data']))
    .toMatchInlineSnapshot(`
      data: { age: 30 }
      status: 'success'
    `);
  ...
  expect(pick(envBFetchEntries[0], ['fields'])).toMatchInlineSnapshot(
    `fields: ['age', 'name']`,
  );
  ...
  expect(envB.timelineString).toMatchInlineSnapshot(`
    ...
    3.82s | ···      | ···       | 🔴 [users||1] >fetch-started
    4.62s | ···      | ···       | 🔴 [users||1] <fetch-finished (value: {"age":30,"name":"Alice"})
    .     | 30       | Alice     | [name-hook, age-hook] ui-changed
    5.43s | 30       | Alice     | [users||1] <confirmed-item-snapshot-received (value: {"name":"Alicia","age":30})
...
test('focused RTU item refetch lets a background tab reuse snapshot fields and refetch only its extra field', async () => {
  ...
  expect(pick(envAHook.result.current, ['status', 'data']))
    .toMatchInlineSnapshot(`
      ...
      status: 'success'
    `);
  expect(pick(envBHooks.result.current.nameAge, ['status', 'data']))
    .toMatchInlineSnapshot(`
      data: { age: 30, name: 'Alice' }
      status: 'success'
    `);
  expect(pick(envBHooks.result.current.city, ['status', 'data']))
    .toMatchInlineSnapshot(`
      data: { city: 'London' }
      status: 'success'
    `);
  expect(
    [...(envB.store.state.itemLoadedFields[itemKey] ?? [])].sort(),
  ).toMatchInlineSnapshot(`['age', 'city', 'name']`);
...
test('focused RTU list refetch lets makes a background tab correctly invalidate the list with extra fields', async () => {
  ...
  renderHook(() => {
    const list = focusedTab.apiStore.useListQuery(
      { tableId: 'users' },
      { returnRefetchingStatus: true, fields: ['name', 'age'], loadSize: 2 },
    );
  ...
  expect(backgroundTab.serverTable.getRequestHistory('list'))
    .toMatchInlineSnapshot(`
      - _type: 'list'
        ...
        returned_items: 2
        time: '3.82s -> 4.62s | duration: 800ms'
      - _type: 'list'
        payload:
          fields: ['name', 'age', 'city']
          pos: { limit: 2, offset: 0 }
...
test('list query partial-resources remote query snapshots satisfy affected field-subset hooks without refetching unaffected sibling hooks', async () => {
  ...
  expect(pick(envBQueries.result.current.name, ['status', 'items']))
    .toMatchInlineSnapshot(`
      items:
        - name: 'Alice'
      status: 'success'
    `);
  expect(pick(envBQueries.result.current.age, ['status', 'items']))
    .toMatchInlineSnapshot(`
      items:
        - age: 30
      status: 'success'
  ...
  expect(envA.timelineString).toMatchInlineSnapshot(`
    "
    time  |
    3.81s | server-data-changed (value: {"id":1,"name":"Alice","age":31})
    3.82s | 🔴 >list-fetch-started
    4.62s | 🔴 <list-fetch-finished (value: {"count":1})
    "
  `);
  expect(envB.timelineString).toMatchInlineSnapshot(`
    "
    ...
    3.01s | -         | -          | 🔴 >list-fetch-started
    3.81s | -         | -          | 🔴 <list-fetch-finished (value: {"count":1})
    .     | 30        | Alice      | [name-query, age-query] ui-initialized
    4.62s | 30        | Alice      | <confirmed-query-snapshot-received (value: {"queryKey":"{tableId:\\"users\\"}","itemCount":1})
...
test('list query partial-resources full-resource list hooks refetch in one tab and update a sibling tab via query snapshots', async () => {
  ...
  const envAQuery = renderHook(() => {
    const query = envA.apiStore.useListQuery(
      { tableId: 'users' },
      { returnRefetchingStatus: true, fields: '*', loadSize: 1 },
    );
  ...
  act(() => {
    envB.serverTable.setItem('users||1', { id: 1, name: 'Alice', age: 31 });
  ...
  expect(pick(envAQuery.result.current, ['status', 'items']))
    .toMatchInlineSnapshot(`
      items:
  ...
  expect(envA.timelineString).toMatchInlineSnapshot(`
    ...
    3.01s | -        | 🔴 >list-fetch-started
    3.81s | -        | 🔴 <list-fetch-finished (value: {"count":1})
    4.62s | -        | <confirmed-query-snapshot-received (value: {"queryKey":"{tableId:\\"users\\"}","itemCount":1})
    .     | Alice:30 | ui-initialized
    5.43s | Alice:30 | <confirmed-query-snapshot-received (value: {"queryKey":"{tableId:\\"users\\"}","itemCount":1})
    .     | Alice:31 | ui-changed
    "
  `);
  expect(envB.timelineString).toMatchInlineSnapshot(`
    ...
    4.63s | Alice:30 | 🟠 >list-fetch-started
    5.43s | Alice:30 | 🟠 <list-fetch-finished (value: {"count":1})
    .     | Alice:31 | ui-changed
    "
  `);
});
...
test('list query partial-resources remote snapshots clear satisfied local invalidation fields for affected and unaffected item hooks', async () => {
  ...
  expect(
    envB.store.state.itemFieldInvalidationFields[itemKey],
  ).toMatchInlineSnapshot(`['age']`);
  ...
  expect(envB.timelineString).toMatchInlineSnapshot(`
    ...
    3.01s | -        | -         | -         | 🔴 [users||1] >fetch-started
    3.81s | -        | -         | -         | 🔴 [users||1] <fetch-finished (value: {"name":"Alice","age":30})
    .     | 30       | Alice:30  | Alice     | [name-hook, age-hook, full-hook] ui-initialized
    4.62s | 30       | Alice:30  | Alice     | [users||1] <confirmed-item-snapshot-received (value: {"age":31})
...
test('a sibling partial snapshot for one field does not clear a local invalidation for a different field', async () => {
  ...
  expect(envB.store.state.itemLoadedFields[itemKeyB]).toMatchInlineSnapshot(
    `[]`,
  );
  expect(
  ...
  envA.serverTable.setItem('users||1', { id: 1, name: 'Alicia', age: 31 });
  void envA.apiStore.scheduleItemFetch('highPriority', 'users||1', {
    fields: ['name'],
  });
  await flushAllTimers();
  ...
  expect(envB.store.state.items[itemKeyB]).toMatchInlineSnapshot(`
    age: 30
    name: 'Alicia'
  `);
  ...
  expect(envA.timelineString).toMatchInlineSnapshot(`
    "
    time  |
    3.81s | server-data-changed (value: {"id":1,"name":"Alicia","age":31})
    3.82s | 🔴 >fetch-started
    4.62s | 🔴 <fetch-finished (value: {"name":"Alicia"})
    4.63s | 🟠 >fetch-started
    5.43s | 🟠 <fetch-finished (value: {"age":31})
    "
  `);
  expect(envB.timelineString).toMatchInlineSnapshot(`
    "
    ...
    4.62s | 30       | Alice     | [users||1] <confirmed-item-snapshot-received (value: {"name":"Alicia"})
    .     | 30       | Alicia    | [name-hook] ui-changed
    5.43s | 30       | Alicia    | [users||1] <confirmed-item-snapshot-received (value: {"name":"Alicia","age":31})
...
test('failed partial-resource list query fetch does not broadcast stale metadata to sibling tab', async () => {
  ...
  expect(pick(envAQuery.result.current, ['status', 'items']))
    ...
    `);
  expect(
    pick(envAQuery.result.current.error, ['message']),
  ).toMatchInlineSnapshot(`message: 'Network error'`);
  expect(pick(envBQuery.result.current, ['status', 'items']))
    .toMatchInlineSnapshot(`
      items:
        - { age: 30, name: 'Alice' }
  ...
  expect(envA.timelineString).toMatchInlineSnapshot(`
    ...
    4.63s | Alice:30 | ···           | refetching   | [query-status] ui-changed
    .     | Alice:30 | ···           | refetching   | 🟠 >list-fetch-started
    5.43s | Alice:30 | ···           | refetching   | 🟠 <list-fetch-error (value: "error")
...
test('failed partial-resource item fetch does not broadcast stale metadata to sibling tab', async () => {
  ...
  expect(envA.store.state.itemLoadedFields[itemKeyA]).toMatchInlineSnapshot(
    `['name']`,
  );
  expect(
  ...
  expect(pick(envAHook.result.current, ['status', 'data']))
    ...
    `);
  expect(
    pick(envAHook.result.current.error, ['message']),
  ).toMatchInlineSnapshot(`message: 'Network error'`);
  expect(envA.store.state.itemLoadedFields[itemKeyA]).toMatchInlineSnapshot(
    `['name']`,
  );
  ...
  expect(envA.timelineString).toMatchInlineSnapshot(`
    ...
    4.62s | -        | -             | -           | [users||1] <confirmed-item-snapshot-received (value: {"name":"Alice","age":30})
    .     | Alice:30 | ···           | success     | [item, item-status, item-error] ui-initialized
    4.63s | Alice:30 | ···           | refetching  | [item-status] ui-changed
  ...
  expect(envB.timelineString).toMatchInlineSnapshot(`
    ...
    time  | item     | item-error | item-status |
    3.82s | -        | -          | -           | 🔴 [users||1] >fetch-started
    4.62s | -        | -          | -           | 🔴 [users||1] <fetch-finished (value: {"name":"Alice","age":30})
    .     | Alice:30 | ···        | success     | [item, item-status, item-error] ui-initialized
    "
  `);
});
