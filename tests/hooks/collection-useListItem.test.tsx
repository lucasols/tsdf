...
describe('collection store useListItemIsDeleted', () => {
  test('item exists after load: returns false', async () => {
    ...
    renderHook(() => {
      const isDeleted = env.apiStore.useListItemIsDeleted('1', {
        itemId: 'a',
...
describe('collection store useListItemIsLoading', () => {
  test('item exists after load: returns false', async () => {
    const env = createCollectionStoreTestEnv<Todo>(
      { '1': defaultTodo },
      { testScenario: 'loaded' },
  ...
  test('item not found triggers fallback after timeout', async () => {
    ...
    // After 100ms the default fallback (invalidateItem) triggers a fetch
...
describe('collection store useListItem (combined)', () => {
  test('returns isLoading, isDeleted, and data correctly after load', async () => {
    ...
    renderHook(() => {
      const result = env.apiStore.useListItem('1', {
        itemId: 'a',
        selector: (data) => data?.value.items['a'],
      });
